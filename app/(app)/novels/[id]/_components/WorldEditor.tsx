        ...
        <div className="mt-12 space-y-10">
          ...
          <Section title="时空背景设定" hint="40-180 字" icon="globe">
            <textarea
              ...
              rows={4}
              className="w-full text-[14px] font-medium text-text-primary bg-secondary/30 border-none rounded-2xl px-5 py-4 leading-relaxed focus:ring-2 focus:ring-primary/20 focus:bg-white transition resize-none shadow-inner"
          ...
          <Section title="底层运行规则" hint={`${world.rules.length} / 10`} icon="rules">
            <div className="space-y-3">
              {world.rules.map((rule, i) => (
                <div key={i} className="flex gap-3 group">
                  <input
                    ...
                    className="flex-1 text-sm font-medium bg-secondary/30 border-none rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary/20 focus:bg-white transition shadow-inner"
              ...
              {world.rules.length < 10 && (
                <button
                  onClick={addRule}
                  className="w-full py-4 bg-white border border-dashed border-border-strong rounded-2xl text-[11px] font-bold text-text-dim hover:border-primary/40 hover:text-primary hover:bg-primary/5 transition shadow-sm"
                >
                  + 增添世界法则
                </button>
              )}
          ...
          <Section title="活跃势力图谱" hint={`${world.factions.length} / 4`} icon="factions">
            <div className="grid gap-6 md:grid-cols-2">
              {world.factions.map((faction, i) => (
                <div key={i} className="card bg-white p-6 rounded-3xl border-border-subtle shadow-sm hover:shadow-md transition space-y-5">
                  ...
                  <div className="grid gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full text-xs font-bold bg-secondary/50 border-none rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-primary/20 focus:bg-white transition shadow-inner"
                      />
                    </div>
                    <div>
                      ...
                      <textarea
                        ...
                        rows={2}
                        className="w-full text-xs font-medium bg-secondary/50 border-none rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-primary/20 focus:bg-white transition shadow-inner resize-none"
              ...
              {world.factions.length < 4 && (
                <button
                  onClick={addFaction}
                  className="h-full min-h-[200px] bg-white border-2 border-dashed border-border-strong rounded-3xl flex flex-col items-center justify-center gap-3 text-text-dim hover:border-primary/40 hover:bg-primary/5 hover:text-primary transition group"
                >
          ...
          <Section title="关键地标 / 地理" hint={`${world.geography.length} / 10`} icon="map">
            <div className="grid gap-3 sm:grid-cols-2">
              {world.geography.map((geo, i) => (
                <div key={i} className="flex gap-2 group">
                  ...
                  {world.geography.length > 2 && (
                    <button
                      onClick={() => removeGeo(i)}
                      className="p-2 text-text-dim hover:text-red-500 opacity-0 group-hover:opacity-100 transition"
                    >
                      <svg aria-hidden="true" className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                      </svg>
              ...
              {world.geography.length < 10 && (
                <button
                  onClick={addGeo}
                  className="py-2.5 border border-dashed border-border-strong rounded-xl text-[11px] font-bold text-text-dim hover:border-primary/40 hover:text-primary transition shadow-sm bg-white"
                >
                  + 添加新地点
                </button>
              )}
